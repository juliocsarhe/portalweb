...
async function getUbicaciones(
  ...
  const url = `http://26.73.68.190:8080/api/v1/admin/sysadmin/ubicaciones/getAll?page=${page}&size=${size}`
...
async function getUbicacionById(token: string, id: number): Promise<UbicacionItem> {
  const res = await fetch(`http://26.73.68.190:8080/api/v1/admin/sysadmin/ubicaciones/${id}`, {
    headers: { Authorization: `Bearer ${token}` },
  })
  if (!res.ok) throw new Error(await res.text())
  return res.json()
...
async function createUbicacion(
  ...
  const res = await fetch(`http://26.73.68.190:8080/api/v1/admin/sysadmin/ubicaciones/create`, {
    method: 'POST',
    headers: {
...
async function updateUbicacion(
  ...
  const res = await fetch(`http://26.73.68.190:8080/api/v1/admin/sysadmin/ubicaciones/update/${id}`, {
    method: 'PUT',
    headers: {
      Authorization: `Bearer ${token}`,
      'Content-Type': 'application/json',
...
async function deleteUbicacion(token: string, id: number): Promise<true> {
  const res = await fetch(`http://26.73.68.190:8080/api/v1/admin/sysadmin/ubicaciones/delete/${id}`, {
    method: 'DELETE',
    headers: { Authorization: `Bearer ${token}` },
  })
  if (!res.ok) throw new Error(await res.text())

...
export default function EditarEquipoPage() {
  ...
  const updateMemberEstado = (id: number, nuevoEstado: 'A' | 'I') => {
    setMiembros((prev) => prev.map((m) => (m.id === id ? { ...m, estado: nuevoEstado } : m)))
  }
      ...
      try {
        ...
        setLeadOptions(
          rawLeads.map((u: any) => ({
            ...
            label:
              [u.nombre, u.apellido].filter(Boolean).join(' ') || u.nombreCompleto || 'Sin nombre',
          }))
        )
          ...
          setMemberOptions(
            notIn.map((u: any) => ({
              ...
              dispRestante: Math.max(
                0,
                Math.min(100, u.disponibilidadRestante ?? u.disponibilidad ?? u.dispRestante ?? 100)
              ),
            }))
          )
            ...
            if (urAll.ok) {
              ...
              usersArr.forEach((u: any) => {
                ...
                const rest = Math.max(
                  ...
                  Math.min(
                    100,
                    u.disponibilidadRestante ?? u.disponibilidad ?? u.dispRestante ?? 100
                  )
                )
        ...
        setLeadId(lId)
        setClienteSel(
          t.cliente?.idCliente ? { value: t.cliente.idCliente, label: t.cliente.nombre } : null
        )
        setLeadSel(
          lId
            ? {
                value: lId,
                label: [t.lider?.nombre, t.lider?.apellido].filter(Boolean).join(' '),
              }
            : null
        )
...
diversity_3
  ...
  const handleFormChangeDeferred = useCallback((updated: Record<string, any>) => {
    Promise.resolve().then(() => setFormData((prev) => ({ ...prev, ...updated })))
  }, [])
  ...
  const updateMemberDisp = (id: number, val: number) =>
    setMiembros((ms) =>
      ms.map((m) => (m.id === id ? { ...m, disponibilidad: Math.max(1, Math.min(100, val)) } : m))
    )
    ...
    },
    {
      ...
      render: (s: IMiembrosEquipo) => (
        <input
          ...
          onChange={(e) =>
            setMiembros((prev) =>
              prev.map((m) => (m.id === s.id ? { ...m, estado: e.target.checked ? 'A' : 'I' } : m))
            )
          }
          className="w-4 h-4"
        />
    ...
    { key: 'nombreDia', label: 'Día' },
    {
      ...
      render: (r: AsignDU) => (
        <Select
          ...
          value={
            r.idUbicacion != null ? { value: r.idUbicacion, label: r.nombreUbicacion ?? '—' } : null
          }
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-end">
              ...
              <div className="grid grid-cols-3 gap-2 items-end">
                <div className="col-span-2">
                  <label className="block text-sm mb-2">Team Leader (opcional)</label>
                  <Select
                    options={leadOptions}
                    value={leadSel}
              ...
              <div className="md:col-span-2">
                ...
                <Select
                  ...
                  onChange={(opt) => {
                    const m = opt
                    setSelectedMember(m)
                    const max = Math.max(0, Math.min(100, m?.dispRestante ?? 100))
                    setNewMemberMax(max)

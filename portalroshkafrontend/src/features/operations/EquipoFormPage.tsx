...
const CREATE_TEAM_PATH = 'http://26.73.68.190:8080/api/v1/admin/operations/team'
...
export default function EquipoFormPage() {
    ...
    ;(async () => {
      try {
        ...
        if (metaRes.ok) {
          ...
          const rawLeads =
            meta.teamleaders ?? meta.teamLeaders ?? meta.lideres ?? meta.usuarios ?? []
          setLeadOptions(
            rawLeads.map((u: any) => ({
        ...
        if (dr.ok) {
          ...
          >()
          opts.forEach((d) => m.set(d.value, { idUbicacion: null, nombreUbicacion: null }))
          setAsignaciones(m)
        }
          ...
          setMemberOptions(
            usersArr.map((u: any) => ({
              ...
              dispRestante: Math.max(
                ...
                Math.min(
                  100,
                  Number(u.disponibilidadRestante ?? u.disponibilidad ?? u.dispRestante ?? 100)
                )
              ),
            }))
          )
  ...
  const handleRemoveTec = (id: number) => setTecSel((prev) => prev.filter((t) => t.value !== id))
  ...
  const rowsDU = useMemo(() => {
    return diasOptions.map((d) => {
      const curr = asignaciones.get(d.value) ?? {
        idUbicacion: null,
  ...
  const handleAddMember = () => {
    ...
    if (newMemberDisp > maxPermitido) {
      setError(
        `El porcentaje asignado (${newMemberDisp}%) supera el máximo permitido (${maxPermitido}%)`
      )
      return
    }

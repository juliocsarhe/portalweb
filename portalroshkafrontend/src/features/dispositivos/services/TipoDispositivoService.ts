...
async function getTiposDispositivo(token: string): Promise<TipoDispositivoItem[]> {
  const res = await fetch(`${import.meta.env.VITE_API_URL}/api/v1/admin/sysadmin/deviceTypes/allTypes`, {
...
async function getTipoDispositivoById(token: string, id: number): Promise<TipoDispositivoItem> {
  const res = await fetch(
    `${import.meta.env.VITE_API_URL}/api/v1/admin/sysadmin/deviceTypes/getTypeDevice/${id}`,
    {
      headers: { Authorization: `Bearer ${token}` },
    }
  )
...
async function createTipoDispositivo(
  ...
  const res = await fetch(
    `${import.meta.env.VITE_API_URL}/api/v1/admin/sysadmin/deviceTypes/createTypeDevice`,
    {
      method: 'POST',
      headers: {
...
async function updateTipoDispositivo(
  ...
  const res = await fetch(
    `${import.meta.env.VITE_API_URL}/api/v1/admin/sysadmin/deviceTypes/updateTypeDevice/${id}`,
    {
      method: 'PUT',
      headers: {
        Authorization: `Bearer ${token}`,
...
async function deleteTipoDispositivo(token: string, id: number): Promise<void> {
  const res = await fetch(
    `${import.meta.env.VITE_API_URL}/api/v1/admin/sysadmin/deviceTypes/deleteTypeDevice/${id}`,
    {
      method: 'DELETE',
      headers: { Authorization: `Bearer ${token}` },
    }

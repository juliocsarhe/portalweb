...
async function getSolicitudesDispositivoAdmin(
  ...
  const url = `${import.meta.env.VITE_API_URL}/api/v1/admin/sysadmin/allRequests?page=${page}&size=${size}`
...
async function getSolicitudesDispositivoUsuario(token: string): Promise<SolicitudUserItem[]> {
  const url = `${import.meta.env.VITE_API_URL}/api/v1/usuarios/solicitudes`
...
async function acceptSolicitudDispositivo(token: string, idSolicitud: number): Promise<void> {
  // 👈 ahora void
  const url = `${import.meta.env.VITE_API_URL}/api/v1/admin/sysadmin/deviceRequest/${idSolicitud}/accept`
...
async function rejectSolicitudDispositivo(token: string, idSolicitud: number): Promise<void> {
  const url = `${import.meta.env.VITE_API_URL}/api/v1/admin/sysadmin/deviceRequest/${idSolicitud}/reject`
...
async function createSolicitudDispositivo(
  ...
  const url = `${import.meta.env.VITE_API_URL}/api/v1/usuarios/pedirdispositivo`
  const res = await fetch(url, {
    method: 'POST',
    headers: {
...
async function getSolicitudById(token: string, idSolicitud: number): Promise<SolicitudUserItem> {
  const url = `${import.meta.env.VITE_API_URL}/api/v1/admin/sysadmin/request/${idSolicitud}`
